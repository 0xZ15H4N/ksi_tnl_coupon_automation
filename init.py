...
# Check time
    if now >= start or now < tm(0, 30):  # Allow from 12:30 AM to 12:30 AM 
        ...
        if metadata[0]:
            print("EMAIL SENDED")
            send_email(sender= os.getenv("EMAIL_"),password = os.getenv("APP_PASS"),recipient =os.getenv("CLIENT"),subject="KSI NEW VIDEO",body=f"WAKE UP! KSI DROPPED NEW VIDEO {metadata[2]} {metadata[3]}")
